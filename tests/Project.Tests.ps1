...
Describe "PSScriptAnalyzer rule-sets" -Tag Build , ScriptAnalyzer {
    ...
    $scripts = Get-ChildItem $ModuleBase -Include *.ps1 -Recurse | Where-Object fullname -notmatch 'classes'
    ...
    foreach ( $Script in $scripts ) {
        Context "Script '$($script.FullName)'" {
            foreach ( $rule in $rules ) {
                ...
                It "Rule [$rule]" {
                    (Invoke-ScriptAnalyzer -Path $script.FullName -IncludeRule $rule.RuleName -ExcludeRule PSUseToExportFieldsInManifest,PSAvoidUsingWriteHost,PSUseSingularNouns,PSUseShouldProcessForStateChangingFunctions -Severity Error,Warning ).Count | Should Be 0
                }
            }
        }
    }
